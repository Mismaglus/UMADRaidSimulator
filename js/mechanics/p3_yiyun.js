...
const P3_SHOCKWAVE = p3mech({ id:'p3_shock', name:'究极冲击波（场边波·8击）',
  setup(){ this.start=Math.floor(this.rng()*8)*45; this.dir=this.rng()<0.5?1:-1; this.subEnd=2;
    this.hitN=0; this.kind='sweep'; this.flash=0; this.SEND=-this.arenaR*0.4; },   // 前沿从场边推进到 SEND(覆盖~70%); 站对面~30%躲
  curAng(){ return d2r(this.start + this.dir*45*this.hitN); },   // 本击来波方向(指向来源场边)
  tick(sdt){ if(this.flash>0) this.flash-=sdt; if(this.kind!=='sweep') return;
    if(this.t>=this.subEnd){ this.t-=this.subEnd; this.doHit(); } },
  doHit(){ const ang=this.curAng(); this.flash=0.4;   // 判定: 已扫到的一侧 (p·D > SEND)
    if(humanRole!=='OB'){ const me=Scene.get(humanRole).pos; if(me[0]*Math.sin(ang)+me[1]*Math.cos(ang) > this.SEND) this.fail('('+humanRole+') 第'+(this.hitN+1)+'击被场边波扫到'); }
    this.hitN++; if(this.hitN>=8) this.kind='done'; },
  extra(d){ if(this.kind!=='sweep') return; const ang=this.curAng();
    const s = this.arenaR - (this.arenaR-this.SEND)*Math.min(1,this.t/this.subEnd);   // 前沿从场边(+R)推进到 SEND，深蓝弓形 clip 在场地内
    d.push({type:'sweepseg',ang:ang,s:s,R:this.arenaR,color:[0.12,0.34,0.82],alpha:this.flash>0?0.5:0.30}); },
