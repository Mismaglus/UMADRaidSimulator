...
const P3_WIND = p3mech({ id:'p3_wind', name:'混沌之顺风 / 逆风', attr:'wind',
  onReset(){ this.windTgt=null; this.dbl=[]; },
  ...
  doKnock(){ this.kind='knock';   // boss 中心击退所有人; startKBv 会按各自顺/逆风+朝向缩放距离并清除该状态
...
const P3_SHOCKWAVE = p3mech({ id:'p3_shock', name:'究极冲击波（场边波·8击）',
  setup(){ this.start=Math.floor(this.rng()*8)*45; this.dir=this.rng()<0.5?1:-1; this.subEnd=1e9;
    this.kind='sweep'; this.hitWave=-1; this.SEND=-this.arenaR*0.5; this.curAng=d2r(this.start); this.curS=this.arenaR; this.flash=0; },   // 危险封顶 SEND(覆盖75%留对面25%); 波前一路推进到对面边(整场)
  tick(sdt){ if(this.flash>0) this.flash-=sdt; if(this.kind!=='sweep') return;
    ...
    const tau=this.t-wave*2; this.curAng=d2r(this.start+this.dir*45*wave); this.curS=this.arenaR*(1-tau);   // 前沿 +R(来源场边) → -R(对面场边), 扫过整场
    if(tau>=1.5 && this.hitWave!==wave){ this.hitWave=wave; this.flash=0.3; const ang=this.curAng;   // 判定: 已扫到的一侧(p·D > SEND)
  ...
  extra(d){ if(this.kind!=='sweep') return;
    d.push({type:'sweepseg',ang:this.curAng,s:this.curS,R:this.arenaR,color:[0.10,0.26,0.66],alpha:0.16});                                  // 波前推进到整场(浅蓝)
    d.push({type:'sweepseg',ang:this.curAng,s:Math.max(this.SEND,this.curS),R:this.arenaR,color:[0.14,0.40,0.9],alpha:this.flash>0?0.5:0.32}); },   // 实际危险(深蓝, 封顶SEND)
...
const P3_FULL = p3mech({ id:'p3_full', name:'★ 一运 · 全程（完整时间轴）',
  ...
  setup(){ this.fxList=[]; this.evi=0; this.shockOn=false; this.t=0; this.subEnd=1e9; this.kind='run'; this.endT=73; this.phaseTxt='深层痛楚'; this.windCleared=false; this._torUntil=0; this._tornadoAt=0; this.dsTgt='MT';
    ...
    this.schedule=[
      ...
      {t:57, lbl:'真空波+本影读·究极冲击波起', fn(){ self.shockOn=true; self.shockStart=Math.floor(self.rng()*8)*45; self.shockDir=self.rng()<0.5?1:-1; self.shockN=0; self.shockNextT=57; self.shockSEND=-self.arenaR*0.4; }},
  ...
  tick(sdt){
    ...
    if(this.shockOn && this.t>=this.shockNextT && this.shockN<8){ var A=d2r(this.shockStart + this.shockDir*45*this.shockN);   // 究极冲击波: 每2s一击共8次, 场边波扫到 SEND 侧
        this.addFx([{type:'sweepseg',ang:A,s:this.shockSEND,R:this.arenaR,color:[0.12,0.36,0.92],alpha:0.5}],0.45);
        if(humanRole!=='OB'){ var me=Scene.get(humanRole).pos; if(me[0]*Math.sin(A)+me[1]*Math.cos(A) > this.shockSEND) this.fail('究极冲击波第'+(this.shockN+1)+'击 ('+humanRole+')'); }
  ...
  elemResolve(pair, elem){ var aoes=[], cr=this.crystalPos[elem];   // 本波两人同属性 elem; 只有对应属性水晶发动
    ...
    P3.nearestN(cr[0],cr[1],2).forEach(function(r){ var tp=Scene.get(r).pos; aoes.push(elem==='fire'?{type:'donut',x:tp[0],z:tp[1],radius:10,color:[1,0.5,0.15],alpha:0.4,kb:true,kbDist:15}:{type:'spread',x:tp[0],z:tp[1],radius:5,color:[0.3,0.7,1],alpha:0.44,kb:true,kbDist:15}); });   // 火→烈焰(环) / 水→海啸(圆), 点最近2; 两者都击退(清顺/逆风并按朝向改距离)
    this.addFx(aoes,1.6);
  ...
  fireVacuum(){ for(var i=0;i<ALL.length;i++){ var r=ALL[i], p=Scene.get(r).pos, away=Math.atan2(p[0],p[1]); startKBv(r, Math.sin(away)*20, Math.cos(away)*20); }   // 中心击退; startKBv 按各自顺/逆风+朝向缩放并清除残余的风(死亡由 tick 统一查 KBOUT)
    this._tornadoAt=this.t+0.7; this.addFx([{type:'charge',x:0,z:0,radius:8}],0.7); },
  extra(d){
