...
const P3_SHOCKWAVE = p3mech({ id:'p3_shock', name:'究极冲击波（旋转半场·8击）',
  setup(){ this.start=Math.floor(this.rng()*8)*45; this.dir=this.rng()<0.5?1:-1; this.subEnd=2;
    this.hitN=0; this.kind='sweep'; this.dispA=d2r(this.start); this.flash=0; this.doHit(); },   // 第1击立即(起点角)
  tick(sdt){ if(this.flash>0) this.flash-=sdt; if(this.kind!=='sweep') return;
    this.dispA += this.dir*(TAU/8)*(sdt/this.subEnd);                 // 持续扫场: 每 subEnd 转 45°
    if(this.t>=this.subEnd){ this.t-=this.subEnd; this.doHit(); } },
  doHit(){ const A=d2r(this.start + this.dir*45*this.hitN); this.dispA=A; this.flash=0.45;   // 本击判定角
    if(humanRole!=='OB'){ const me=Scene.get(humanRole).pos; if(me[0]*Math.sin(A)+me[1]*Math.cos(A) > 0.5) this.fail('('+humanRole+') 第'+(this.hitN+1)+'击站在波动半场内'); }
    this.hitN++; if(this.hitN>=8) this.kind='done'; },
  extra(d){ if(this.kind==='sweep') d.push({type:'halfcleave',x:0,z:0,radius:36,facing:this.dispA,color:[0.12,0.30,0.72],alpha:this.flash>0?0.52:0.26}); },   // 深蓝半场波
  hudLines(){ const L=['P3·一运 — 究极冲击波（旋转半场·每2s共8击）  种子:'+this.seed];
    if(this.kind==='sweep') L.push('起点 '+this.start+'° · '+(this.dir>0?'顺时针':'逆时针')+' · 已 '+this.hitN+'/8 击','站波动【对侧】半场, 同向跟着转(45°/击)');
...
const P3_FULL = p3mech({ id:'p3_full', name:'★ 一运 · 全程（完整时间轴）',
  ...
  tick(sdt){
    ...
    if(this.shockOn){ this.dispA=d2r(this.shockStart + this.shockDir*45*((this.t-57)/2));   // 究极冲击波: 持续旋转 + 每2s一击共8次
      if(this.t>=this.shockNextT && this.shockN<8){ var A=d2r(this.shockStart + this.shockDir*45*this.shockN);
        this.addFx([{type:'halfcleave',x:0,z:0,radius:36,facing:A,color:[0.12,0.30,0.72],alpha:0.5}],0.4);
        if(humanRole!=='OB'){ var me=Scene.get(humanRole).pos; if(me[0]*Math.sin(A)+me[1]*Math.cos(A)>0.5) this.fail('究极冲击波第'+(this.shockN+1)+'击 ('+humanRole+')'); }
  ...
  extra(d){
    ...
    if(this.shockOn) d.push({type:'halfcleave',x:0,z:0,radius:36,facing:this.dispA,color:[0.12,0.30,0.72],alpha:0.22});
  ...
  buffsOf(role){ var out=[], e=this.elem&&this.elem[role];
    ...
    var w=this.windType&&this.windType[role]; if(w && !this.windCleared && this.t<68) out.push({label:w==='wind'?'风':'逆风',color:w==='wind'?[0.45,0.9,0.55]:[0.55,0.8,0.98],rem:Math.max(0,68-this.t),kind:'debuff'});
    return out; },
  hudLines(){ var L=['P3·一运 ★全程  场地30m  种子:'+this.seed, '⏱ '+this.t.toFixed(0)+'s · '+this.phaseTxt+' · 速度可调(2×更快)'];
    if(humanRole!=='OB'){ var me=[]; if(this.elem[humanRole]) me.push(P3.ATTR[this.elem[humanRole]].cn); if(this.windType[humanRole]&&!this.windCleared) me.push(this.windType[humanRole]==='wind'?'风(真空波背对中心)':'逆风(真空波正对中心)'); if(me.length) L.push('你: '+me.join(' + ')); }
